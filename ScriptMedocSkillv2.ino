...
const int DEFAULT_COMPARTMENT = 1;
...
Badge badges[] = {
  {{0x35, 0xC2, 0xEB, 0x76}, 2},
  {{0x34, 0x45, 0x16, 0x06}, 3},
  {{0xA4, 0x70, 0xB3, 0x85}, 4},
  {{0x7E, 0x9B, 0x5A, 0x6C}, 2},  // ← à changer si besoin
};

const int NUM_BADGES = sizeof(badges) / sizeof(badges[0]); // ← automatique, plus d'erreur
...
int getCompartment(byte* uid) {
  for (int i = 0; i < NUM_BADGES; i++) {  // ← utilise NUM_BADGES au lieu de 4
    bool match = true;
    for (byte j = 0; j < 4; j++) {
